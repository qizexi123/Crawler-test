...
class JobSalaryUtils:
    ...
    async def find_job_salary_back_markdown(self, job_code: str, job_name: str) -> Dict[str, str]:
        """
        通过岗位名称/代码进行检索
        ...
        url = f"{self.base_url_query}{job_code}"
        if not rs and type(rs) != list:
            return {"query": job_code, "result": "", "url": url}
...
{lines}
"""
        return {"query": job_code, "result": markdown, "url": url}
