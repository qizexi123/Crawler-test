...
async def search_and_save_page(url: str, search_keyword: str) -> bool:
    async with async_playwright() as p:
        # 启动浏览器
        browser = await p.chromium.launch(headless=False)  # 设置为True可在后台运行
...
async def main():
    ...
    # # 单个公司处理
    # await search_and_save_page(url, "000001")
